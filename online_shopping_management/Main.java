...
public class Main {
	...
	private static void addProduct() {
		try {
			...
			Product newProduct = new Product(productManagement.generateProductId(), name, description, price, category,
					quantity);
	...
	private static void placeOrder() {
		try {
			...
			Product product = productManagement.getProductById(productId);
			if (product != null && product.getQuantity() >= quantity) {
				Order newOrder = new Order(orderManagement.generateOrderId(), loggedInUser.getId(), productId, quantity,
						product.getPrice() * quantity, "Pending", address);
